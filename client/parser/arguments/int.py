...
class IntArgument:
  ...
  def parse(self, arguments: Sequence[str], start_index: int) -> tuple[int | None, int]:
    ...
    try:
      value = int(arguments[start_index])
    except ValueError:
      raise ArgumentParseError(f"Invalid integer value for argument '{self.name}': {arguments[start_index]}")
